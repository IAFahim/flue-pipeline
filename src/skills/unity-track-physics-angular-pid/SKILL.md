...
bodies, Targets slots, and stat setups are OTHER specialists' domains (protocol §6: report "no
egg", never improvise). **Family patterns live in `unity-track-physics-filter-override`**
...
return t == null ? "NO_EGG|PhysicsAngularPIDTrack not found - package BovineLabs.Timeline.Physics absent" : "OK|" + t.AssemblyQualifiedName + "|dataPath=" + UnityEngine.Application.dataPath;
```
