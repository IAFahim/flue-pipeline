...
description: "Designer's map of WHERE core gameplay lives in a BovineLabs \"Arvex\" project and how to add to it: the Assets/Settings tree (Schemas + ObjectDefinitions) and the three auto-maintained registries (EssenceSettings = stats+intrinsics, ReactionSettings = events, ObjectManagementSettings = objdefs). Covers the auto-register + auto-ID mechanism (create the asset in the right folder; the editor assigns the key and adds it to the registry — never hand-edit the array) plus the duplicate/wrong-key and two-StaggerMeter traps. Use when a designer asks \"where do I add a new stat/counter/event/spawnable?\" or \"why isn't my new stat showing up in the dropdown?\" Portable; the folders and IDs are a vex-ee worked example to rediscover in THIS project. Cross-references unity-stats-intrinsics, unity-reactions, unity-object-definitions, unity-targets, unity-essence-actions, unity-tra-payloads, unity-augment-architecture."
---
